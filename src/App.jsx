...
const CustomBBICon = ({ size = '24px', className = '' }) => (
  <span className={`inline-flex items-center justify-center font-extrabold text-white rounded-full bg-gradient-to-r from-blue-600 to-purple-600 ${className}`} style={{ width: size, height: size, fontSize: `calc(${size} / 2)`, lineHeight: 1, transform: 'translateY(-1px)' }}>
    bb
  </span>
);
    ...
    <section id="hero" className="flex flex-col lg:flex-row items-center justify-center gap-16 min-h-[60vh] scroll-mt-32 overflow-hidden">
      <div className="flex-1 space-y-6 animate-fade-in-up text-left max-w-xl">
        ...
        <h1 className="text-5xl md:text-7xl font-extrabold tracking-tight text-slate-900 dark:text-white leading-tight">
          Data Driven. <br />
          <span className="bg-gradient-to-r from-blue-600 via-indigo-500 to-cyan-400 bg-clip-text text-transparent animate-gradient-text pb-2">Cloud Native.</span>
        </h1>
        ...
        <div className="flex flex-wrap items-center gap-4 pt-2">
          ...
          <div className="flex gap-3 pl-2 border-l border-slate-300 dark:border-slate-700 ml-2">
            ...
            <a href={resumeData.personalInfo.github} target="_blank" rel="noreferrer" className="p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors" aria-label="GitHub Profile">
              <Github size={25} />
            </a>
          </div>
        </div>
      ...
      <div className="hidden lg:flex w-72 h-72 relative shrink-0 items-center justify-center">
         ...
         <div className="relative z-10 grid grid-cols-2 gap-4 place-content-center">
            <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-xl border border-slate-300 dark:border-slate-700 flex items-center justify-center animate-float" style={{ animationDelay: '0s' }}><Database className="text-blue-500" size={32} /></div>
            <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-xl border border-slate-300 dark:border-slate-700 flex items-center justify-center animate-float" style={{ animationDelay: '1.5s' }}><BarChart3 className="text-cyan-500" size={32} /></div>
            <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-xl border border-slate-300 dark:border-slate-700 flex items-center justify-center animate-float" style={{ animationDelay: '3s' }}><Server className="text-indigo-500" size={32} /></div>
            <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-xl border border-slate-300 dark:border-slate-700 flex items-center justify-center animate-float" style={{ animationDelay: '4.5s' }}><Cloud className="text-violet-500" size={32} /></div>
         </div>
      </div>
    </section>
  );
...
const ImpactSection = ({ onClose }) => {
  ...
  return (
    <section id="impact" className="scroll-mt-24 animate-fade-in-up relative pt-10 pb-6 border-t border-b border-slate-100 dark:border-slate-800/50 my-10 bg-slate-50/50 dark:bg-slate-900/30 -mx-4 px-4 sm:mx-0 sm:px-0 sm:bg-transparent sm:dark:bg-transparent sm:border-0 rounded-3xl">
      ...
      <SectionHeader icon={TrendingUp} title="Engineering Impact" colorClass="bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400" />
        ...
        <div className="lg:col-span-3 grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-2xl shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
            ...
            <div className="relative z-10">
              ...
              <div className="text-sm font-semibold text-green-600 dark:text-green-400 uppercase tracking-wide">Refresh Time Reduction</div>
              <p className="text-xs text-slate-500 mt-2">Migrated legacy MSAS cubes to optimized Databricks pipelines.</p>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-2xl shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
            ...
            <div className="relative z-10">
              ...
              <div className="text-sm font-semibold text-blue-600 dark:text-blue-400 uppercase tracking-wide">Finance Users Scaled</div>
              <p className="text-xs text-slate-500 mt-2">Supporting enterprise analytics across multiple finance divisions.</p>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-2xl shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
            ...
            <div className="relative z-10">
              ...
              <div className="text-sm font-semibold text-purple-600 dark:text-purple-400 uppercase tracking-wide">Trade Value Supported</div>
              <p className="text-xs text-slate-500 mt-2">Ensured continuous data support for USMCA compliance reporting.</p>
            </div>
          </div>
        </div>

        {/* Visual 1: Optimization Chart */}
          ...
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={performanceData} layout="vertical" margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                ...
                <YAxis dataKey="name" type="category" width={140} tick={{fill: '#64748b', fontSize: 12, fontWeight: 'bold'}} />
                <Tooltip 
...
const ExperienceSection = ({ resumeData }) => (
  <section id="experience" className="scroll-mt-24">
    <SectionHeader icon={Trophy} title="Professional Experience" colorClass="bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400" />
      ...
      {resumeData.experience.map((job, idx) => (
        <div key={idx} className="relative pl-8 md:pl-12">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-2">
            ...
            <div className="text-sm font-medium text-slate-500 bg-slate-100 dark:bg-slate-900 px-3 py-1 rounded-full self-start sm:self-auto">{job.period}</div>
          </div>
...
const ProjectsSection = ({ resumeData }) => {
  return (
    <section id="projects" className="scroll-mt-24">
      <SectionHeader icon={Gem} title="Key Projects" colorClass="bg-cyan-100 dark:bg-cyan-900/30 text-cyan-600 dark:text-cyan-400" />
          ...
          <div key={idx} className="group flex flex-col bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm hover:shadow-xl hover:border-blue-500/30 transition-all duration-300">
            ...
            <div className="flex flex-wrap gap-2 pt-4 border-t border-slate-100 dark:border-slate-800">
              {project.tech.map((t, tIdx) => (
                <span key={tIdx} className="px-2 py-1 text-xs font-medium rounded bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 group-hover:bg-blue-50 dark:group-hover:bg-blue-900/20 group-hover:text-blue-600 dark:group-hover:text-blue-300 transition-colors">{t}</span>
              ))}
            </div>
          </div>
        ))}
...
const SkillsSection = ({ resumeData }) => {
  return (
    <section id="skills" className="scroll-mt-24">
      <SectionHeader icon={Cpu} title="Technical Skills" colorClass="bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400" />
      
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {resumeData.skills.map((skillGroup, idx) => {
            const Icon = skillGroup.icon;
            return (
              <div key={idx} className="bg-slate-50 dark:bg-slate-900/50 rounded-2xl p-5 border border-slate-200 dark:border-slate-800 hover:border-indigo-400 dark:hover:border-indigo-500 transition-colors group">
                ...
                <div className="flex flex-wrap gap-2">
                  {skillGroup.items.map((item, iIdx) => (
                    <span key={iIdx} className="px-2.5 py-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-md text-xs font-medium text-slate-600 dark:text-slate-300 shadow-sm">
                      {item}
                    </span>
                  ))}
                </div>
      ...
      <div className="space-y-6">
        {resumeData.leadership.map((lead, idx) => (
          <div key={idx} className="group pl-4 border-l-2 border-slate-200 dark:border-slate-800 hover:border-cyan-500 transition-colors">
            ...
            <p className="text-sm text-slate-600 dark:text-slate-400 mt-2 leading-relaxed">{lead.description}</p>
          </div>
        ))}
        <div className="group pl-4 border-l-2 border-slate-200 dark:border-slate-800 hover:border-cyan-500 transition-colors">
            ...
            <div className="text-sm text-slate-600 dark:text-slate-400 mt-2 leading-relaxed">{resumeData.activities.join(' • ')}</div>
        </div>
      </div>
    </div>
    {/* Awards */}
...
const BioSection = () => (
  <section id="bio" className="scroll-mt-24 text-center">
    <SectionHeader icon={User} title="About Me" colorClass="bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400" />
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 max-w-4xl mx-auto">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 shadow-sm hover:shadow-md transition-shadow flex flex-col items-center gap-3 text-center">
        <div className="p-2 bg-orange-100 dark:bg-orange-900/30 rounded-full text-orange-600 dark:text-orange-400"><Dumbbell size={20} /></div>
        <p className="text-sm text-slate-600 dark:text-slate-300 leading-snug">Former college athlete, current home gym enthusiast.</p>
      </div>
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 shadow-sm hover:shadow-md transition-shadow flex flex-col items-center gap-3 text-center">
        <div className="p-2 bg-blue-100 dark:bg-blue-900/30 rounded-full text-blue-600 dark:text-blue-400"><Gamepad2 size={20} /></div>
        <p className="text-sm text-slate-600 dark:text-slate-300 leading-snug">Competitive FPS player and lifelong MMO/RPG fan.</p>
      </div>
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 shadow-sm hover:shadow-md transition-shadow flex flex-col items-center gap-3 text-center">
        <div className="p-2 bg-red-100 dark:bg-red-900/30 rounded-full text-red-600 dark:text-red-400"><MousePointer2 size={20} /></div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 shadow-sm hover:shadow-md transition-shadow flex flex-col items-center gap-3 text-center">
        <div className="p-2 bg-green-100 dark:bg-green-900/30 rounded-full text-green-600 dark:text-green-400"><Dog size={20} /></div>
    ...
    <div className="max-w-full px-4 sm:px-6 lg:px-8 h-full">
      <div className="flex justify-between items-center h-full">
        <div className="flex items-center gap-4">
          <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="p-2 -ml-2 rounded-lg text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors" aria-label="Toggle Menu">
            {isSidebarOpen ? <X size={24} /> : <Menu size={24} />}
          </button>
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => scrollToSection('hero')}>
            <CustomBBICon size="32px" />
            <span className="hidden md:inline text-xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent ml-2">Brady Barker</span>
          </div>
        </div>
        <div className="flex items-center gap-4">
          <button onClick={() => scrollToSection('hero')} className="hidden md:flex items-center gap-2 px-4 py-2 text-sm font-medium text-slate-600 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors">
            <Home size={18} /> Home
          </button>
          <button onClick={() => setDarkMode(!darkMode)} className="p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors border border-transparent hover:border-slate-200 dark:hover:border-slate-700" aria-label="Toggle Dark Mode">
...
const MobileNav = ({ scrollToSection }) => (
  <div className="md:hidden fixed bottom-0 left-0 right-0 border-t bg-white dark:bg-slate-950 border-slate-200 dark:border-slate-800 z-50 px-4 py-3 flex justify-between items-center overflow-x-auto no-scrollbar">
     {[{id: 'experience', label: 'Exp.', icon: Trophy}, {id: 'projects', label: 'Projects', icon: Gem}, {id: 'skills', label: 'Skills', icon: Cpu}, {id: 'education', label: 'Edu.', icon: GraduationCap}, {id: 'bio', label: 'Bio', icon: User}].map(item => (
       <button key={item.id} onClick={() => scrollToSection(item.id)} className="text-xs font-medium text-slate-600 dark:text-slate-400 flex flex-col items-center gap-1 min-w-[60px]">
...
const App = () => {
  ...
  const [showImpact, setShowImpact] = useState(false); // NEW STATE for showing impact
  ...
  useEffect(() => {
    ...
    const sections = ['hero', 'experience', 'projects', 'impact', 'skills', 'education', 'bio'];
    sections.forEach((id) => {
  ...
  }, [showImpact]); // Re-run when layout changes (impact shown)
  ...
  const SideNavItem = ({ id, label, icon: Icon }) => (
    <button onClick={() => scrollToSection(id)} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-300 text-left ${activeSection === id ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 border-l-4 border-blue-600 dark:border-blue-400' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 border-l-4 border-transparent'}`}>
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${darkMode ? 'bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-900'}`}>
      ...
      {/* Sidebar */}
      <aside className={`fixed left-0 top-16 bottom-0 w-64 border-r transition-transform duration-300 ease-in-out z-40 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} ${darkMode ? 'bg-slate-950 border-slate-800' : 'bg-white border-slate-200'}`}>
        <div className="p-6 space-y-2 h-full overflow-y-auto">
          <div className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-4 px-4">Sections</div>
          <SideNavItem id="experience" label="Experience" icon={Trophy} />
          <SideNavItem id="projects" label="Key Projects" icon={Gem} />
          {showImpact && <SideNavItem id="impact" label="Engineering Impact" icon={TrendingUp} />}
      ...
      {/* Main Content */}
      <main className="pt-24 pb-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-24 transition-all duration-300 ease-in-out" style={{ transform: 'translateZ(0)' }}>
